...
function setupUserAssignment(frm, doctype, docname) {
	...
	frappe.call({
		method: "assignmate.assignmate.api.get_active_users_list",
		args: {
	...
	frappe.call({
		method: "assignmate.assignmate.api.is_user_assigned",
		args: {
			doctype: doctype,
			docname: docname,
		...
		callback: function (r) {
			...
			if (isAssigned || isAdmin) {
				...
				else if (isAdmin) frm.set_intro("Administrator can manage assignments", "blue");
...
function showAssignmentDialog(doctype, docname, initialUsers, userDataLoaded) {
	const dialog = new frappe.ui.Dialog({
		...
		primary_action: function () {
			...
			frappe.call({
				method: "assignmate.assignmate.api.assign_doc_to_users",
				args: {
					doctype: doctype,
					docname: docname,
					users: selectedUsers,
	...
	// Show loading state
	const selectedContainer = $(dialog.fields_dict.selected_users_section.wrapper).find(
		"#selected-users-container"
	);
	const listContainer = $(dialog.fields_dict.users_list_section.wrapper).find(
		"#users-list-container"
	);
	selectedContainer.html('<div class="text-muted" style="padding:10px;">Loading...</div>');
	listContainer.html('<div class="text-muted" style="padding:10px;">Loading users...</div>');
	...
	function renderSelectedUsers() {
		...
		if (selectedUsers.length === 0) {
			html = '<div class="text-muted" style="padding:10px;">No users selected</div>';
		} else {
		...
		selectedContainer.find(".remove-selected-user").on("click", function () {
			...
			// Remove from backend
			frappe.call({
				method: "assignmate.assignmate.api.remove_doc_assigned_user",
				args: {
					doctype: doctype,
					docname: docname,
					users: [userId],
	...
	dialog.fields_dict.search_user.$input.on("input", function () {
		...
		searchTimeout = setTimeout(() => {
			listContainer.html('<div class="text-muted" style="padding:10px;">Searching...</div>');
...
// // Add assignment to specific doctypes
...
// 	refresh: function (frm) {
// 		setupUserAssignment(frm, "Issue", frm.doc.name);
// 	},
// });

// frappe.ui.form.on("ToDo", {
// 	refresh: function (frm) {
// 		setupUserAssignment(frm, "ToDo", frm.doc.name);
// 	},
// });
